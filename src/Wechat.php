...
class Wechat{
    ...
    public function __construct($options = array()) {
        ...
        $this->pemCret      = isset($options['pemcret'])       ? $options['pemcret']      : '';
        $this->pemKey       = isset($options['pemkey'])        ? $options['pemkey']       : '';
     ...
     *
     */
    public function qrcode($scene_id = '', $limit = true, $expire = 1800) {
        /*
    ...
    /**
     * 发送客服消息
     * @param msgtype 包括 text ,image,video,music,news
     * @return boolean
     *
     */
     ...
     * @return boolean;
     */
    public function sync_news($arr) {
        $params = $this->json_encode($arr);
        $url = self::SYNC_NEWS . '?access_token=' . $this->access_token;
        $jsonStr = $this->http($url, $params, 'POST');
        $jsonArr = $this->parseJson($jsonStr);
        if ($jsonArr) {
     ...
     * @return boolean;
     */
    public function sync_image($arr) {
        $params = $arr;
     ...
     * @return boolean;
     */
    public function sync_list($arr){
        $params = $this->json_encode($arr);
        $url = self::SYNC_LIST . '?access_token=' . $this->access_token;
        $jsonStr = $this->http($url, $params, 'POST');
        $jsonArr = $this->parseJson($jsonStr);
        if ($jsonArr) {
            return $jsonArr;
    ...
    public function unifiedOrder($openid, $body, $orderId, $money, $notify_url = '', $extend = array()) {
        ...
        $params = array(
            ...
            'total_fee'        => intval($money * 100), // 转换成分
            'spbill_create_ip' => ip(),
        ...
        if ($data) {
            if ($data['return_code'] == 'SUCCESS') {
                if ($data['result_code'] == 'SUCCESS' && $params['trade_type']=='JSAPI') {
                    return $this->createPayParams($data['prepay_id']);
                }else if($data['result_code'] == 'SUCCESS' && $params['trade_type']=='NATIVE'){
     ...
     * 扫码支付获得短地址
     */
    public function scanurl($product_id){
        $info['product_id'] = $product_id;
        $info['appid'] = $this->appid;
        $info['mch_id'] = $this->mch_id;
        $info['time_stamp'] = time();
        $info['nonce_str'] = self::_getRandomStr();
        $info['sign'] = self::_getOrderMd5($info);
        $long_url = "weixin://wxpay/bizpayurl?sign=".$info['sign']."&appid=".$info['appid']."&mch_id=".$info['mch_id']."&product_id=".$info['product_id']."&time_stamp=".$info['time_stamp']."&nonce_str=".$info['nonce_str'];
        $url = 'https://api.mch.weixin.qq.com/tools/shorturl';
        $params['appid'] = $this->appid;
        $params['mch_id'] = $this->mch_id;
        $params['long_url'] = urlencode($long_url);
        $params['nonce_str'] = self::_getRandomStr(20);
        $params['sign'] = self::_getOrderMd5($params);
        $data = self::_array2Xml($params);
        $data = $this->http($url,$data,'POST');
        return self::parsePayRequest($data);
    }
    /**
     * 接受扫码通知
    ...
    public function getscaninfo(){
        $data = self::_extractXml($GLOBALS["HTTP_RAW_POST_DATA"]);
        if (empty($data)) {
     ...
     *   扫码下单
     */
    public function scanorder($openid,$product_id, $body, $orderId, $money, $notify_url = ''){
        if (strlen($body) > 127) $body = substr($body, 0, 127);
        $params = array(
            'appid'            => $this->appid,
            'mch_id'           => $this->mch_id,
        ...
        if ($data) {
            if ($data['return_code'] == 'SUCCESS') {
                if($data['result_code'] == 'SUCCESS' && $params['trade_type']=='NATIVE'){
                    $row['return_code'] = "SUCCESS";
                    $row['result_code'] = "SUCCESS";
                    $row['appid'] = $this->appid;
                    $row['mch_id'] = $this->mch_id;
                    $row['nonce_str'] = self::_getRandomStr();
                    $row['prepay_id'] = $data['prepay_id'];
                    $row['sign'] = self::_getOrderMd5($row);
                    return self::_array2Xml($row);
                } else {
                    $this->error = $data['err_code'];
                    return false;
                }
     ...
     * author luoyt
     */
    public function sendGroupred($info = array()){
        $params = array(
            'nonce_str'=>$this->_getRandomStr(18),
            'mch_id'=>$this->mch_id,
            'mch_billno'=>$this->getredno(),
            'wxappid'=>$this->appid,
            'amt_type'=>'ALL_RAND'
        );
        $params = array_merge($params,$info);
        //$params['sign'] = $this->getSign($params);
        $params['sign'] = $this->_getOrderMd5($params);
        $data = self::_array2Xml($params);
        $data = $this->http(self::SEND_GROUP_RED, $data, 'POST',true);
        $data = self::_extractXml($data);
        return $data;
    }
     ...
     * author luoyt
     */
    public function sendred($info = array()){
        $params = array(
            'nonce_str'=>$this->_getRandomStr(18),
            'mch_id'=>$this->mch_id,
            'mch_billno'=>$this->getredno(),
        ...
        $params['sign'] = $this->_getOrderMd5($params);
        $data = self::_array2Xml($params);
        $data = $this->http(self::SEND_RED, $data, 'POST',true);
        $data = self::_extractXml($data);
        return $data;
    }
     ...
     * 企业转账
     */
    public function sendmoney($info = array()){
        $params = array(
            'mch_appid'=>$this->appid,
            'mchid'=>$this->mch_id,
            'nonce_str'=>$this->_getRandomStr(18),
            'partner_trade_no'=>$this->getredno(),
        ...
        $params['sign'] = $this->_getOrderMd5($params);
        $data = self::_array2Xml($params);
        $data = $this->http(self::SEND_MMPAY, $data, 'POST',true);
        $data = self::_extractXml($data);
        return $data;
    }

    //得到红包订单号
    private function getredno(){
        return $this->mch_id.date('Ymd').'0'.sprintf("%09d", mt_rand(1, 999999999));
    }
    ...
    private function image($image){
        $image = array('MediaId'=>$image);
        $this->data['Image'] = $image;
    }

    /**
     * 回复视频
     * @param  string $content 要视频
     *
     */
    private function video($video){
        $video = array('MediaId'=>$video);
    ...
    private function http($url, $params = array(), $method = 'GET', $ssl = false){
        ...
        if ($ssl) {
            if(empty($this->pemCret)){
                $this->error = "证书不存在！";
                return false;
            }
            $opts[CURLOPT_SSLCERT]     = $this->pemCret;
            $opts[CURLOPT_SSLKEY]      = $this->pemKey;
            $opts[CURLOPT_CAINFO]      = $this->rootca;
        }
        /* 初始化并执行curl请求 */
        $ch = curl_init();
    ...
    private function createPayParams($prepay_id) {
        ...
        $params['signType']  = 'MD5';
        $params['paySign'] = $this->_getOrderMd5($params);
     ...
     * 解析支付接口的返回结果
     */
    private function parsePayRequest($data) {
        $data = $this->_extractXml($data);
        if (empty($data)) {
            $this->error = '支付返回内容解析失败';
            return false;
        }
        if (!$this->_checkSign($data)) return false;
        // 有返回结果 并且是SUCCESS的时候
        if ($data['return_code'] == 'SUCCESS') {
    ...
    private function getNotify() {
        ...
        return $this->parsePayRequest($data);
    }
    /**
     * 对支付回调接口返回成功通知
     */
    private function returnNotify($return_msg = true) {
        if ($return_msg == true) {
            $data = array(
                'return_code' => 'SUCCESS',
            );
     ...
     * @return xml|boolean
     */
    private function AESdecode($encrypted) {
     ...
     * @return boolean
     */
    private function AESencode($text) {
        $key    = base64_decode($this->AESKey . "=");
     ...
     * @return string
     */
    private static function createOrderId($prefix = '') {
     ...
     * 对数据进行SHA1签名
     */
    private function getSHA1($encrypt_msg, $nonce = '') {
